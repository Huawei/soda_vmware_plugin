...
public class OpenSDSStorageAdapter implements IPluginAdaptor {
    ...
    private static final Logger log = Logger
            .getLogger(OpenSDSStorageAdapter.class);

    private OpenSDSStorageFactory factory;

    /**
     * Create a new plugin factory. You could use the username / password to
     * create a factory for each user. in this case, we will always use the same
     * as the adapter is not user dependent.
     *
     * @param sessionID is the session ID asking for the element. Sessions are created
     * for each UI and for each user executing workflows
     * @param username is the user requesting the objects.
     * @param password password associated with the username
     * @param notificationHandler is used by the plugin to notify the UI an element has changed,
     * was deleted, etc.
     ...
     */
    @Deprecated
    public void installLicenses(PluginLicense[] licenses)
        throws PluginLicenseException {
        // Not used with the current license system.
    }

    /**
     * Allow to register event publisher used by the policy Engine Not used in
     * this sample
     */
    ...
    /**
     * used by the server to destroy a previously created factory. If you need
     * to do some specific cleaning when the factory is release, put the code
     * from here. Not used in this sample
     */
