...
public class OpenSDSStorageEventGenerator {
    private static final Logger log = Logger
            .getLogger(OpenSDSStorageEventGenerator.class);
    /**
     * OpenSDSStorage Event Generator Object
     */
    public static final OpenSDSStorageEventGenerator NEW_OPEN_SDS_STORAGE_EVENT_GENERATOR =
            new OpenSDSStorageEventGenerator();
     ...
     */
    public static OpenSDSStorageEventGenerator createScriptingSingleton(
    ...
    /**
     * add policy element
     * @param sdkType disk type
     * @param id disk id
     * @param publisher publisher
     */
    public void addPolicyElement(String sdkType, String id,
     ...
     * @param publisher publisher
     */
    public boolean removePolicyElement(String sdkType, String id,
        IPluginEventPublisher publisher) {
        String key = sdkType + "' / '" + id;
        if (log.isInfoEnabled()) {
     ...
     */
    public void generateEvent(String sdkType, String id, double magnitude) {
        String key = sdkType + "' / '" + id;
        if (log.isInfoEnabled()) {
            log.info("Generate Flare Event for : '" + key + "' with magnitude "
                    + magnitude);
        }
        Vector<IPluginEventPublisher> publishers = policyElements.get(key);
        if (publishers != null) {
            for (IPluginEventPublisher publisher : publishers) {
                publisher.pushGauge(sdkType,
                        id,
                        "Flare",
                        "magnitude",
                        magnitude);
